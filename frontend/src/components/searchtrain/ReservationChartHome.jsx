...
const ReservationChartHome = () => {
    ...
    const [chart, setChart] = useState(JSON.parse(sessionStorage.getItem('reservationChart')) || null);
    ...
    useEffect(() => {
        if (error) {
            if (!isAuthenticatedUser) {
                sessionStorage.setItem('reservationChart', JSON.stringify(formik.values));
        ...
        if (reservationChart?.length > 0) {
            formik.resetForm();
        }
    }, [dispatch, error, reservationChart]);

    useEffect(() => {
        if (chart) {
            dispatch(getReservationChart(chart));
            sessionStorage.removeItem('reservationChart')
        }
    ...
    return (
        <div className="bg-white pt-8 border rounded">
                ...
                <section className="flex flex-col gap-3 md:gap-2 relative lg:p-2">
                    ...
                    <div className="text-center p-2 mt-4">
                        <button type='submit' onClick={formik.handleSubmit}
                            className="bg-gray-900 text-white px-8 py-2 md:py-3 rounded text-md font-semibold">Get
                            Train Chart</button>
                    </div>
...
export default ReservationChartHome