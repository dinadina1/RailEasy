...
import { getStationById, newStation, updateStationById } from '../../../actions/stationAction';
import toast from 'react-hot-toast';
import { clearIsStationCreated, clearIsStationUpdated, clearStationError } from '../../../slices/stationSlice';
import { useParams } from 'react-router-dom';

const EditStation = ({ setBgImage }) => {

    const dispatch = useDispatch();
    const { id } = useParams();

    const { loading } = useSelector(state => state.authState);
    const { error, station, isUpdated, loading: stationLoading } = useSelector(state => state.stationState);
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            dispatch(updateStationById(station._id, formData));
        }
    });
    ...
    useEffect(() => {
        ...
        if (isUpdated) {
            ...
            formik.resetForm();
            dispatch(clearIsStationUpdated());
        }
    }, [error, isUpdated]);
    ...
    useEffect(() => {
        if (station) {
            formik.setValues({
                ...
                total_platform: station?.total_platform,
                city: station?.city,
                state: station?.state,
    ...
    return (
        <>
            <MetaData title={`Edit station - ${id}`} />
            {
                loading ? <Loader /> : (
                            ...
                            <div className="container mx-auto p-4 max-w-lg">
                                <form onSubmit={formik.handleSubmit}>
                                        ...
                                        <select type="text" id="country" placeholder="Enter country" name='country'
                                            ...
                                            <option value="" defaultValue>--Select country--</option>
                                            {
                                                getNames().map((country, index) => (
                                                    <option key={index} value={country.toUpperCase()}>{country}</option>
                                                ))
                                            }
                                        </select>
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <select type="text" id="status" placeholder="Enter status" name='status'
                                        ...
                                        {/* display error if status is invalid */}

                                        {
                                            formik.errors.status && formik.touched.status ? (
                                                <span className="text-red-500 text-sm flex gap-1">
                                                    ...
                                                    <span className='my-auto'>{formik.errors.status}</span>
                                                </span>
                                            ) : null
                                        }
                                    </div>
