...
import { getParticularTrain } from '../../../actions/trainAction';

const TrainDetail = ({ setBgImage }) => {

    const { id } = useParams();
    const dispatch = useDispatch();

    const { loading, user } = useSelector(state => state.authState);
    const { train } = useSelector(state => state.trainState);
                    ...
                    <main className='flex'>
                        ...
                        <div className="max-w-6xl mx-auto bg-white rounded-xl shadow-xl overflow-auto">
                            <div className="flex justify-between p-3">
                                {/* <!-- Back button --> */}
                                <Link to={"/admin/train/all"} className="max-w-sm p-3 flex text-blue-500 font-bold text-xl">
                            ...
                            <div className="m-2 md:m-6 p-4 bg-gradient-to-br from-blue-50 to-indigo-50">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    ...
                                    <div className="text text-gray-700 space-y-2">
                                        <p><strong>Departure Time:</strong> {train?.route?.length && train?.route[0]?.departure_time}</p>
                                        <p><strong>Arrival Time:</strong> {train?.route?.length && train?.route[train?.route.length - 1]?.arrival_time}</p>
